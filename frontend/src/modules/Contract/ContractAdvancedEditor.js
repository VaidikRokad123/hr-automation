...
import './ContractAdvancedEditor.css';
import { useLocation, useNavigate } from 'react-router-dom';
import apiClient from '../../api/client';
import { sanitizeHtml } from '../../utils/safeHtml';
import {
  CONTRACT_BOTTOM_PADDING_MM,
  CONTRACT_CONTENT_HEIGHT_MM,
  CONTRACT_LETTERHEAD_STYLE,
  CONTRACT_PACKING_LIMIT_MM,
  CONTRACT_SIDE_PADDING_MM,
  CONTRACT_TOP_PADDING_MM
} from './contractLayout';

function ContractAdvancedEditor() {
  ...
  const [previewPageScale, setPreviewPageScale] = useState(0.48);

  // Contract mode constants
  const PAGE_HEIGHT_MM = 297;
  const ACTIVE_TOP_PADDING_MM = CONTRACT_TOP_PADDING_MM;
  const ACTIVE_BOTTOM_SAFE_MM = CONTRACT_BOTTOM_PADDING_MM;
  const ACTIVE_SIDE_PADDING_MM = CONTRACT_SIDE_PADDING_MM;
  const ACTIVE_CONTENT_HEIGHT_MM = CONTRACT_CONTENT_HEIGHT_MM;
  const PAGE_PACKING_LIMIT_MM = CONTRACT_PACKING_LIMIT_MM;
  const PX_PER_MM = 96 / 25.4;
  const CONTRACT_PAGE_WIDTH_PX = 210 * PX_PER_MM;

  const stripHtml = (text = '') => String(text).replace(/<[^>]*>/g, '');
  const resolveEditorVariables = useCallback((text = '') => String(text).replace(/\$\{([^}]+)\}/g, (_, key) => {
  ...
  const previewPages = useMemo(() => (
    pages.map((page, pageIndex) => ({
      ...
      paragraphs: (page.paragraphs || []).map((paragraph) => ({
        ...paragraph,
        content: (paragraph.type || 'paragraph') === 'image'
          ? paragraph.content
          : resolveEditorVariables(paragraph.content)
      }))
    }))
  ), [pages, resolveEditorVariables]);

  const renderContractPreviewParagraph = (paragraph, index) => {
    const type = paragraph?.type || 'paragraph';
    const content = paragraph?.content || '';

    if (type === 'image') {
      return (
        <div key={paragraph.id || index} className="contract-viewer-image">
    ...
    const classNameByType = {
      date: 'contract-viewer-date',
      subject: 'contract-viewer-subject',
      signature: 'contract-viewer-signature',
      company: 'contract-viewer-company',
      separator: 'contract-viewer-separator',
      to: 'contract-viewer-to',
      footer: 'contract-viewer-paragraph',
      paragraph: 'contract-viewer-paragraph'
    };

    return (
      <div
        key={paragraph.id || index}
        className={classNameByType[type] || 'contract-viewer-paragraph'}
  ...
  const measureParagraphsHeightMm = (paragraphs = []) => {
    ...
    contentEl.style.padding = `${ACTIVE_TOP_PADDING_MM}mm ${ACTIVE_SIDE_PADDING_MM}mm ${ACTIVE_BOTTOM_SAFE_MM}mm ${ACTIVE_SIDE_PADDING_MM}mm`;
    contentEl.style.position = 'relative';
    contentEl.style.height = `${ACTIVE_CONTENT_HEIGHT_MM}mm`;
  ...
  const fetchContractData = useCallback(async () => {
    try {
      ...
      if (response.data.success) {
        const { metadata: backendMetadata } = response.data.data;
  ...
  }, [initialMetadata, initialPages, location.state?.pdfUrl, selectedWorker]);
  ...
  useEffect(() => {
    const updatePreviewScale = () => {
      ...
      const availableWidth = Math.max(0, previewStage.clientWidth - 36);
      const nextScale = Math.min(1, availableWidth / CONTRACT_PAGE_WIDTH_PX);
      setPreviewPageScale(Number.isFinite(nextScale) && nextScale > 0 ? nextScale : 0.48);
    };

    updatePreviewScale();

    const previewStage = previewStageRef.current;
    const resizeObserver = typeof ResizeObserver !== 'undefined' && previewStage
      ? new ResizeObserver(updatePreviewScale)
      : null;

    if (resizeObserver && previewStage) {
  ...
  if (loading) {
    return (
      <div className="ContractAdvancedEditor">
        <div className="loading-container">
          <h2>Preparing contract editor...</h2>
        </div>
      </div>
    );
  }
    ...
    <div className="ContractAdvancedEditor">
      ...
      <div className="editor-container">
        ...
        <div className="preview-section">
          <h3>Employee PDF View</h3>
          <div
            className="contract-editor-preview"
            ref={previewStageRef}
            style={{ '--contract-editor-page-scale': String(previewPageScale) }}
            aria-label="Contract preview"
          >
            {previewPages.map((page, pageIndex) => (
              <div
                key={page.pageNumber || pageIndex}
                className={`contract-editor-preview-page-shell ${currentPageIndex === pageIndex ? 'active' : ''}`}
                onClick={() => setCurrentPageIndex(pageIndex)}
                aria-label={`Contract page ${pageIndex + 1}`}
              >
                <section
                  className="contract-page contract-editor-preview-page"
...
export default ContractAdvancedEditor;
